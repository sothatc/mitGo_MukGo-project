...
<body>
	...
	<div class="notice-write-wrap">
        ...
        <div class="notice-write-frm">
            <form action="/updateQna.do" method="post" enctype="multipart/form-data" class="updateFrm">
            	<input type="hidden" name="qnaNo" value="${qna.qnaNo }">
                <table class="notice-tbl">
                    <tr style="height: 70px; border-bottom: 1px solid rgba(224, 224, 224, 0.7);">
                        <th>*제목</th>
                        <td style="width: 700px;">
                            <input class="w3-input w3-border w3-round-large" id="noticeTitle" name="qnaTitle" type="text" value="${qna.qnaTitle }" style="width: 630px; margin: 0 auto;">
                        </td>
                    </tr>
                    ...
                    <tr style="height: 70px; border-bottom: 1px solid rgba(224, 224, 224, 0.7);" class="secret">
                        ...
                        <td>
                        	<c:choose>
                        		<c:when test="${qna.secretStatus == 1 }">
                        			<input type="checkbox" name="secretStatus" id="secretStatus" checked="checked">
                        		</c:when>
                    ...
                    <c:choose>
                    	<c:when test="${qna.secretStatus == 0 }">
                    	...
                    	<c:otherwise>
                    		<tr style="height: 70px; border-bottom: 1px solid rgba(224, 224, 224, 0.7);" class="secretPw">
		                        <th>비밀번호</th>
		                        <td>
		                            <input class="w3-input w3-border w3-round-large" id="qnaPassword" name="qnaPassword" value="${qna.qnaPassword }" type="text" style="width: 300px; margin: 0 auto;">
		                        </td>
		                    </tr>
                    	</c:otherwise>
                    </c:choose>
                    ...
                    <c:choose>
                    	<c:when test="${not empty qna.fileList }">
                    		<tr style="height: 70px; border-bottom: 1px solid rgba(224, 224, 224, 0.7);" class="fileListTr">
		                    	<th>첨부파일목록</th>
		                    	<td>
		                    		<c:forEach items="${qna.fileList }" var="qf">
		                    			<div class="fileDiv">
		                    				<span>${qf.filename }</span><span class="delManWow" onclick="delFileMan(this, ${qf.qnaFileNo},'/resources/upload/qna/${qf.filepath }')">x</span>
		                    			</div>
		                    		</c:forEach>
		                    	</td>
		                    </tr>
                    ...
                    <tr style="height: 400px;">
                        ...
                        <td style="text-align: left;">
                            <div class="form-floating" style="padding-top: 30px; padding-bottom: 30px">
                                <textarea class="form-control" name="qnaContent" placeholder="Leave a comment here" id="floatingTextarea2" style="height: 340px">
                                	${qna.qnaContent }
                                </textarea>
    ...
    <script>
	    function delFileMan(obj, qnaFileNo, qnaFilepath){
	    	const inputNo = $("<input>");
	    	inputNo.attr("name", "fileNoList");
	    	inputNo.val(qnaFileNo);
	    	inputNo.hide();
	    	
	    	const inputPath = $("<input>");
	    	inputPath.attr("name", "filepathList");
	    	inputPath.val(qnaFilepath);
	    	inputPath.hide();
	    	
	    	$(".updateFrm").append(inputNo).append(inputPath);
